...
/*   Updated: 2025/02/07 15:44:46 by ehedeman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
template <typename T> bool _operator(T lv, T rv);
template <typename T> 
...
template <typename T>
void						PmergeMe::mergeInsertionSort(T &array, int sorting_level)
{
	...
	{
		to_append.insert(to_append.end(), next(array.begin(), sorting_level * (i - 1) - 1));
	...
	for (int k = 2;; k++)
	{
		...
		if (_jnd > static_cast<int>(to_append.size()))
			break ;
		int _not = _jnd;
		typename std::vector<Iterator>::iterator pendIt = next(to_append.begin(), _jnd - 1);
		typename std::vector<Iterator>::iterator bountIt = next(main.begin(), _cjn + _in);
		while (_not)
		{
			typename std::vector<Iterator>::iterator idx = \
				std::upper_bound(main.begin(), bountIt, *pendIt, _operator<Iterator>);
	...
	{
		typename std::vector<Iterator>::iterator _cp = next(to_append.begin(), i);
	...
	if (odd_number >= 0)
	{
		...
		typename std::vector<Iterator>::iterator idx = 
			std::upper_bound(main.begin(), main.end(), temp_it, _operator<Iterator>);
...
template <typename T> void PmergeMe::copy_to_s(T &container)
{
	typename T::iterator it = container.begin();
	this->_s.erase(_s.begin(), _s.end());
	while (it != container.end())
	{
		_s.push_back(*it);
		it++;
